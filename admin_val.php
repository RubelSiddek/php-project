...
<?php 
	$name = $email = $pwd = $re_pwd = $gender = $length = $date ="";
	$nameErr = $emailErr = $pwdErr = $re_pwdErr = $genderErr = "";
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if($valid){
			...
			$sql = "INSERT INTO tbl_admin (id, name, email, pwd, gender, date) VALUES ('', '$name', '$email', '$pwd', '$gender', '$date')";

			if ($conn->query($sql) === TRUE) {
				echo '<div class="alert alert-info">
				  <center><strong>Congratulation!</strong> You are successfully Connected</center>
				</div>';
			}
			else {
